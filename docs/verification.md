...
macOS 설정 내보내기: 전체 Go 테스트·vet·race와 Windows 교차 빌드 통과. 가상 키로 ZIP 내 구성/키 권한, 미선택 키 미포함, 송신 PC 키 경로 제거, 다중 경유·로컬 포워딩 생성을 검증했습니다. 생성한 설정을 Linux OpenSSH `ssh -G`로 연결 없이 해석하고 설치 스크립트를 `bash -n`으로 검사했습니다. macOS 실기기 실행 검증은 수행하지 않았습니다.
