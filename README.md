...
아래 화면은 공개용 가상 데이터로 촬영했습니다. 실제 운영 호스트, 접속 주소, 계정 및 개인 키 경로는 포함하지 않습니다.

![대시보드](docs/images/dashboard.jpg)

등록된 SSH 호스트를 선택하면 서비스용 터널을 함께 만들 수 있습니다.

![서비스 등록](docs/images/add-service.jpg)

Keys의 추가·수정 화면에서 경로를 직접 입력하거나 **파일 선택…** 버튼으로 Windows 파일 선택 창을 열 수 있습니다. 선택한 파일의 경로만 입력하며, 취소하면 기존 경로를 유지합니다. 키 파일을 업로드하거나 복사하지 않습니다.
...
터널 선택 목록에는 기존 터널과 모든 등록 SSH 호스트가 표시됩니다. 연결 상태와 관계없이 호스트를 선택하고 Local port, 대상 주소·포트를 입력하면 새 터널과 서비스가 함께 저장됩니다. 대상 주소는 선택한 SSH 호스트에서 접근하는 주소입니다. 저장만으로 SSH 연결을 시작하지 않으며, 실제 접속에는 올바른 인증 정보와 대상 서비스의 실행 상태가 필요합니다.
