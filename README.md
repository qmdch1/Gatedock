...
- **Windows:** 공유 주소 접속 → **키·호스트 포함 실행파일 다운로드** → 실행. 키와 관련 호스트·터널이 자동 등록됩니다.
- **Mac:** 공유 주소 접속 → **macOS 설정 묶음 다운로드** → 압축 해제. 해당 폴더에서 `bash install.command`를 실행하고 `README.txt`의 접속 명령을 사용합니다.

기존 설정은 덮어쓰지 않습니다. 처음 접속하는 서버의 지문은 확인이 필요할 수 있습니다. 키 포함 파일은 신뢰하는 팀원에게만 전달하고 공개 저장소에는 올리지 마세요.
