...
1. **팀 공유 → 공유 시작**을 누릅니다. 기본으로 전체 키와 관련 호스트·터널을 포함합니다.
2. **주소 복사**를 눌러 같은 로컬망의 팀원에게 전달합니다.

일부만 공유하려면 **공유할 키**를 펼쳐 선택하세요. 공유 중에는 **선택 내용 적용**으로 변경하고, 끝나면 **공유 중지**를 누르면 됩니다. 전체 해제하면 개인 키 없이 설정만 공유합니다.
