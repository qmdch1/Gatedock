...
for name in filter(None, paths):
    ...
    if any(p in {".tools", ".test-data", ".ssh", "secrets", "exports", "backups", "keys", "uploaded-keys", "uploaded-settings", "local", "dist"} for p in path.parts) or re.search(r"(?:\.pem|\.key|\.ppk|\.p12|\.pfx|\.db(?:-.*)?|\.sqlite.*|\.json|\.exe)$", name, re.IGNORECASE) or path.name.startswith(".env"):
